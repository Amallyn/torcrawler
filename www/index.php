...
if (isset($_GET['search'])) { # we have a search request, let's process it
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, "mode=raw&query=SELECT id, url, highlight({}, 'title') title, highlight({}, 'body') body FROM rt WHERE MATCH('{$_GET['search']}') LIMIT 100"); /* here we are SELECTing :
 - url
 - highlighted title
 - highlighted body
...
*/
    if ($json = json_decode(curl_exec($ch))) { # running the query and decoding the JSON
        foreach ($json->data as $result) echo "<small>{$result->url}  - id {$result->id}</small><br><a href=\"{$result->url}\">{$result->title}</a><br>{$result->body}<br><br>"; # and here we just output the results: url, title and body
    }
}
